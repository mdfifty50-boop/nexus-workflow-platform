...
Respond as ${agent.displayName} would naturally respond, maintaining their personality and expertise throughout.`
}
  ...
  async runDiscussionRound(
    ...
    for (const agent of agents) {
      ...
      const referencedAgents = agents
        .filter(a => a.id !== agent.id && response.text.includes(a.displayName))
        .map(a => a.id)
