...
import { AutopilotPageDetector, type PageDetectionResult } from './AutopilotPageDetector.js'
...
const SERVICE_URLS: Record<string, { dashboard: string; setup: string }> = {
  gmail: {
...
export class AutopilotEngine extends EventEmitter {
  ...
  private pageDetector: AutopilotPageDetector = new AutopilotPageDetector()
   ...
   */
  createSession(workflowSpec: WorkflowSpec, userId: string): AutopilotSession {
    ...
    try {
      ...
      browserSession.timeoutTimer = setTimeout(() => {
        ...
        this.cancelSession(sessionId).catch(err =>
          console.error(`[AutopilotEngine] Error cancelling timed-out session:`, err)
        )
      }, SESSION_TIMEOUT_MS)
   ...
   */
  getSession(sessionId: string): AutopilotSession | undefined {
   ...
   */
  private planSteps(workflowSpec: WorkflowSpec): AutopilotStep[] {
  ...
  private async executeSteps(sessionId: string): Promise<void> {
    ...
    while (session.currentStep < session.steps.length) {
      ...
      try {
        ...
        if ((step.status as string) === 'credential_wait') {
  ...
  private async executeStep(
    ...
    if (recheck.isLoginPage || recheck.is2FAPage) {
      ...
      this.emitStatus(sessionId, 'credential_wait',
        `Please log in to ${this.formatServiceName(step.service)}`)
      return
    }
   ...
   */
  private startScreenshotCapture(sessionId: string): void {
    const browserSession = this.browserSessions.get(sessionId)
    if (!browserSession) return

    // Don't start if already running
    if (browserSession.screenshotInterval) return
   ...
   */
  private stopScreenshotCapture(sessionId: string): void {
    const browserSession = this.browserSessions.get(sessionId)
    if (!browserSession?.screenshotInterval) return
  ...
  private emitStatus(sessionId: string, state: AutopilotState, message: string): void {
    this.emit('status', { sessionId, state, step: this.sessions.get(sessionId)?.currentStep ?? 0, message, timestamp: Date.now() })
  }

  private emitProgress(sessionId: string, step: number, message: string): void {
   ...
   */
  private getSessionOrThrow(sessionId: string): AutopilotSession {
    const session = this.sessions.get(sessionId)
    if (!session) {
      throw new Error(`Session ${sessionId} not found`)
    }
  ...
  private cleanupExpiredSessions(): void {
    ...
    for (const [sessionId, session] of Array.from(this.sessions.entries())) {
      ...
      if (isTerminal && isExpired) {
        ...
        this.cancelSession(sessionId).catch(err =>
          console.error(`[AutopilotEngine] Error force-cancelling expired session:`, err)
        )
      }
    }
  }
