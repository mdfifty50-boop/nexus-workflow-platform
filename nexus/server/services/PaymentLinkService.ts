...
export type PaymentProvider = 'myfatoorah' | 'knet' | 'mock'
export type PaymentStatus = 'pending' | 'paid' | 'expired' | 'refunded' | 'failed'
export type SupportedCurrency = 'KWD' | 'USD' | 'SAR' | 'AED' | 'BHD'
...
export interface PaymentRecord {
  ...
  expiresAt: string
  callbackUrl?: string
  providerRef?: string // External reference from the provider
...
interface MyFatoorahSendPaymentRequest {
...
interface MyFatoorahSendPaymentResponse {
  IsSuccess: boolean
  Message: string
  Data: {
    InvoiceId: number
...
interface MyFatoorahPaymentStatusResponse {
  ...
  Data: {
    InvoiceId: number
    InvoiceStatus: 'Pending' | 'Paid' | 'Expired' | 'Canceled'
    InvoiceValue: number
    CustomerName?: string
    CustomerMobile?: string
    CustomerEmail?: string
...
class PaymentLinkServiceClass {
  ...
  private myFatoorahApiKey: string | undefined
  private myFatoorahBaseUrl: string
   ...
   */
  async generatePaymentLink(options: PaymentLinkOptions): Promise<{
    success: boolean
    ...
    try {
      ...
      let result: { paymentUrl: string; providerRef?: string }
   ...
   */
  listPayments(filters?: PaymentListFilters): PaymentRecord[] {
   ...
   */
  async confirmPayment(paymentId: string, providerData?: Record<string, unknown>): Promise<{
    success: boolean
    payment?: PaymentRecord
    error?: string
  }> {
    ...
    if (!payment) {
      ...
      const found = Array.from(this.payments.values()).find(
        p => p.providerRef === paymentId || p.orderId === paymentId
      )
      if (!found) {
   ...
   */
  markFailed(paymentId: string, reason?: string): PaymentRecord | undefined {
    const payment = this.payments.get(paymentId)
   ...
   */
  formatPaymentMessage(payment: PaymentRecord, lang: 'en' | 'ar' = 'en'): string {
   ...
   */
  formatReceiptMessage(payment: PaymentRecord, lang: 'en' | 'ar' = 'en'): string {
    const amount = this.formatAmount(payment.amount, payment.currency)
   ...
   */
  private resolveProvider(requested?: PaymentProvider): PaymentProvider {
    if (requested) {
    ...
    if (this.myFatoorahApiKey) return 'myfatoorah'
    return 'mock'
  }
   ...
   */
  private async generateMyFatoorahLink(
    paymentId: string,
    ...
    const requestBody: MyFatoorahSendPaymentRequest = {
    ...
    const data = await response.json() as MyFatoorahSendPaymentResponse
   ...
   */
  private async checkMyFatoorahStatus(invoiceId: string): Promise<{
    status: PaymentStatus
    paidAt?: string
  } | null> {
    if (!this.myFatoorahApiKey) return null
    ...
    try {
      ...
      const data = await response.json() as MyFatoorahPaymentStatusResponse
   ...
   */
  private async generateKNETLink(
    paymentId: string,
    options: PaymentLinkOptions,
   ...
   */
  private generateMockLink(
    paymentId: string,
    options: PaymentLinkOptions,
    currency: SupportedCurrency,
  ): { paymentUrl: string; providerRef?: string } {
   ...
   */
  private formatAmount(amount: number, currency: SupportedCurrency): string {
  ...
  getStats(): {
    ...
    const byStatus: Record<PaymentStatus, number> = { pending: 0, paid: 0, expired: 0, refunded: 0, failed: 0 }
    const byProvider: Record<PaymentProvider, number> = { myfatoorah: 0, knet: 0, mock: 0 }
...
setInterval(() => {
  ...
  for (const [id, payment] of (paymentLinkService as any).payments as Map<string, PaymentRecord>) {
    if (payment.status === 'pending' && new Date(payment.expiresAt) < now) {
      payment.status = 'expired'
      ;(paymentLinkService as any).payments.set(id, payment)
      expired++
    }
  }
