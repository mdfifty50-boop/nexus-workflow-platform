...
const SERVICE_PATTERNS: Record<string, { loginUrl: RegExp; successUrl: RegExp; twoFactorHints: string[] }> = {
  google: {
...
export class AutopilotPageDetector {
   ...
   */
  async detect(page: Page): Promise<PageDetectionResult> {
    const url = page.url()
   ...
   */
  private async hasLoginText(page: Page): Promise<boolean> {
    try {
   ...
   */
  private async check2FA(page: Page, hints: string[]): Promise<boolean> {
    try {
      const bodyText = await page.textContent('body') || ''
   ...
   */
  private async checkGeneric2FA(page: Page): Promise<boolean> {
    try {
      const bodyText = await page.textContent('body') || ''
   ...
   */
  private async findLoginForm(page: Page): Promise<string | undefined> {
    for (const selector of GENERIC_LOGIN_SELECTORS) {
      try {
        const el = await page.$(selector)
   ...
   */
  private guessServiceFromUrl(url: string): string | null {
    try {
