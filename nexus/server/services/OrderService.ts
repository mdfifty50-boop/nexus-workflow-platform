...
class OrderServiceClass {
   ...
   */
  isOrderMessage(message: string): boolean {
  ...
  async extractOrderFromMessage(
    ...
    customerPhone: string,
    customerName?: string,
    catalogueItems?: Array<{ name: string; price?: number; currency?: string }>
...
- Currency defaults to KWD (Kuwaiti Dinar)`
   ...
   */
  updateSheetRowId(orderId: string, rowId: number): void {
    const order = this.orders.get(orderId)
    if (order) {
   ...
   */
  formatOrderConfirmation(order: Order, lang: 'en' | 'ar' = 'en'): string {
    if (lang === 'ar') {
