...
if (!hasValidCredentials) {
  console.warn('[errorRecoveryService] Missing or invalid Supabase credentials. Error recovery disabled.')
}
...
export interface WorkflowError {
  ...
  retry_count: number
  resolved: boolean
  resolution_method: 'auto_retry' | 'manual_fix' | 'skipped' | null
  created_at: string
}
...
const errorTranslationCache = new Map<string, { plainEnglish: string; fixes: SuggestedFix[] }>()

function getCacheKey(errorType: string, errorMessage: string): string {
...
export const errorRecoveryService = {
  ...
  matchErrorPattern(
    errorMessage: string
  ): { type: string; plainEnglish: string; fixes: Omit<SuggestedFix, 'id'>[] } {
  ...
  ): Promise<{ plainEnglish: string; fixes: Omit<SuggestedFix, 'id'>[] }> {
    try {
      ...
      const claudeResult = await tieredCalls.extract(
        'You are a helpful assistant that translates technical errors into plain English for non-technical users.',
        `Translate this technical error into plain English that a non-technical business user can understand. Also suggest 1-3 potential fixes.
...
Respond in JSON format:
{
  ...
  async executeWithRetry<T>(
  ...
  ): Promise<{ success: boolean; result?: T; error?: WorkflowError }> {
    ...
    // Record final error
    const workflowError = await this.recordError(
      workflowId,
      nodeId,
  ...
  // Get Workflow Errors
  // ---------------------------------------------------------------------------
  async getWorkflowErrors(workflowId: string): Promise<WorkflowError[]> {
  ...
  // ---------------------------------------------------------------------------
  async getUnresolvedErrors(workflowId: string): Promise<WorkflowError[]> {
    const { data } = await supabase
      .from('workflow_errors')
      .select('*')
      .eq('workflow_id', workflowId)
  ...
  async markResolved(
    errorId: string,
    method: 'auto_retry' | 'manual_fix' | 'skipped'
  ...
  async applySuggestedFix(
    ...
    try {
      ...
      const fix = errorData.suggested_fixes?.find((f: SuggestedFix) => f.id === fixId)
