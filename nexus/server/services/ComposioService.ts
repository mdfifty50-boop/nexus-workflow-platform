...
// App definitions for the simple integrations page
...
class ComposioServiceClass {
  ...
  private userConnections: Map<string, Set<string>> = new Map() // userId -> connected apps
    ...
    try {
      ...
      if (accounts && Array.isArray(accounts.items)) {
        accounts.items.forEach((acc) => {
          ...
          const toolkitSlug = (acc as Record<string, unknown>).toolkitSlug as string | undefined
          if (toolkitSlug) {
   ...
   */
  async checkConnection(appName: string): Promise<ConnectionStatus> {
    if (!this.composio) {
      return {
        toolkit: appName,
        connected: false,
    ...
    try {
      ...
      const expiredAccounts = accounts?.items?.filter(
   ...
   */
  async initiateConnection(appName: string, redirectUrl?: string): Promise<{ authUrl?: string; error?: string }> {
    if (!this.composio) {
    ...
    try {
      ...
      if (authConfigs?.items && authConfigs.items.length > 0) {
        ...
        const result = await this.composio.connectedAccounts.initiate(
          this.userId,
   ...
   */
  async getToolkitActions(toolkitName: string): Promise<string[]> {
    if (!this.composio) {
    ...
    try {
      ...
      const actionNames = tools.map((tool: { slug?: string; name?: string }) =>
        tool.slug || tool.name || ''
      ).filter(Boolean)
   ...
   */
  async getTools(apps?: string[]): Promise<ToolInfo[]> {
    if (!this.composio) return []
    ...
    try {
      ...
      return toolsList.map((tool: { slug: string; name: string; description?: string }) => ({
        slug: tool.slug,
        name: tool.name,
   ...
   */
  private generateDemoResponse(toolSlug: string, params: Record<string, unknown>): unknown {
    ...
    if (slug.includes('SHEETS_APPEND') || slug.includes('SHEETS_UPDATE') || slug.includes('GOOGLESHEETS_APPEND')) {
      return {
        ...
        updatedRows: (params.values as unknown[][])?.length || 1,
        updatedCells: 10,
        demoMode: true,
      }
    }
   ...
   */
  async getUserConnections(userId: string): Promise<{ app: string; connected: boolean; connectedAt?: string }[]> {
    if (!this.composio) {
   ...
   */
  async disconnectUserApp(userId: string, appName: string): Promise<{ success: boolean; error?: string }> {
    if (!this.composio) {
    ...
    try {
      ...
      return accounts.items.map((acc) => {
        ...
        return {
          ...
          userId: record.entityId as string || record.userId as string || 'unknown',
          toolkit: (record.toolkitSlug as string || '').toLowerCase(),
   ...
   */
  async listAuthConfigs(): Promise<Array<{
    id: string
    toolkit: string
    type: string
  }>> {
    ...
    try {
      ...
      return configs.items.map((config) => {
        ...
        if (typeof record.toolkit === 'string') {
          ...
          const tk = record.toolkit as Record<string, unknown>
          toolkitStr = (tk.slug as string) || (tk.name as string) || ''
        ...
        return {
          ...
          type: record.type as string || record.authScheme as string || 'unknown',
        }
      })
    } catch (error) {
