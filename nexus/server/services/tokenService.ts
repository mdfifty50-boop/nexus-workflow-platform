...
if (!hasValidCredentials) {
  console.warn('[tokenService] Missing or invalid Supabase credentials. Token tracking disabled.')
}
...
const MODEL_PRICING: Record<string, { input: number; output: number }> = {
  ...
  // Legacy Claude 3 entries (kept for old log compatibility)
...
export const tokenService = {
  ...
  async getOptimizationRecommendations(
    ...
    // Analyze model usage
    const models = Object.entries(summary.byModel) as [string, { tokens: number; cost: number }][]
    ...
    if (opusUsage && opusUsage[1].cost > summary.totalCost * 0.5) {
      recommendations.push(
        'Consider using Claude Sonnet for routine tasks. Opus is best reserved for complex reasoning tasks.'
      )
    }
    ...
    if (!haikuUsage || haikuUsage[1].cost < summary.totalCost * 0.1) {
      recommendations.push(
        'Claude Haiku is great for simple classifications and extractions at 1/12th the cost of Sonnet.'
      )
    }
    ...
    if (summary.inputTokens > summary.outputTokens * 3) {
      recommendations.push(
        'High input token usage detected. Consider summarizing context or using RAG to reduce input size.'
      )
    }
    ...
    if (summary.efficiency === 'poor') {
      recommendations.push(
        'Your cost per operation is high. Review workflow complexity and consider batching similar operations.'
      )
    }

    // Operation-specific recommendations
    const operations = Object.entries(summary.byOperation) as [string, { tokens: number; cost: number }][]
    operations.forEach(([op, usage]) => {
      if (usage.cost > summary.totalCost * 0.3) {
        recommendations.push(
          `The "${op}" operation accounts for ${((usage.cost / summary.totalCost) * 100).toFixed(0)}% of costs. Consider optimizing this workflow.`
        )
      }
    })
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        'Your token usage is well-optimized. Keep up the efficient workflows!'
      )
    }
