...
let cachedInsights: AdminInsights | null = null
...
async function aggregateUserData(): Promise<AggregatedData | null> {
  ...
  try {
    ...
    const userProfiles = (profiles || []).map((p: any) => ({
    ...
    const activeUsersLast7d = userProfiles.filter(u =>
      u.lastActiveAt && new Date(u.lastActiveAt) >= sevenDaysAgo
    ).length
    const activeUsersLast30d = userProfiles.filter(u =>
    ...
    const businessProfiles = (bizProfiles || []).map((b: any) => ({
    ...
    const uniqueConvUsers = new Set((convUsers || []).map((c: any) => c.clerk_user_id))
    ...
    const uniqueWfUsers = new Set((wfData || []).map((w: any) => w.clerk_user_id))
...
async function analyzeWithClaude(data: AggregatedData): Promise<AdminInsights> {
  if (!anthropic) {
...
function emptyInsights(reason: string): AdminInsights {
  const now = new Date()
  return {
    segments: [],
    featureAdoption: [],
...
export const adminInsightsEngine = {
   ...
   */
  async getInsights(forceRefresh: boolean = false): Promise<AdminInsights> {
   ...
   */
  async generateInsights(): Promise<AdminInsights> {
    if (!supabase) {
