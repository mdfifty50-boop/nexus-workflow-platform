...
export const projectService = {
   ...
   */
  async getProjectsForUser(clerkUserId: string): Promise<Project[]> {
    if (!supabase) {
      console.error('Supabase not configured')
      return []
    }
   ...
   */
  async getProjectById(projectId: string, clerkUserId: string): Promise<Project | null> {
    if (!supabase) {
      console.error('Supabase not configured')
      return null
    }
   ...
   */
  async archiveProject(projectId: string, clerkUserId: string): Promise<boolean> {
    if (!supabase) {
      console.error('Supabase not configured')
      return false
    }
   ...
   */
  async restoreProject(projectId: string, clerkUserId: string): Promise<boolean> {
    if (!supabase) {
      console.error('Supabase not configured')
      return false
    }
   ...
   */
  async getArchivedProjects(clerkUserId: string): Promise<Project[]> {
    if (!supabase) {
      console.error('Supabase not configured')
      return []
    }
  ...
  async getUserProfileByClerkId(clerkUserId: string) {
   ...
   */
  getDevUserProfile(clerkUserId: string) {
   ...
   */
  createDevProject(data: CreateProjectData, userProfile: any): Project {
   ...
   */
  getDevProjects(userId: string): Project[] {
   ...
   */
  async createUserProfile(clerkUserId: string) {
    if (!supabase) return null
  ...
  async userCanEditProject(projectId: string, userId: string): Promise<boolean> {
    if (!supabase) return false
  ...
  async userIsProjectOwner(projectId: string, userId: string): Promise<boolean> {
    if (!supabase) return false
   ...
   */
  async getProjectMembers(projectId: string, clerkUserId: string): Promise<any[]> {
    if (!supabase) return []
  ...
  async userCanManageMembers(projectId: string, userId: string): Promise<boolean> {
    if (!supabase) return false
   ...
   */
  async getProjectAnalytics(projectId: string, clerkUserId: string): Promise<any | null> {
    if (!supabase) return null
    ...
    const totalCostUsd = executions?.reduce((sum, e) => sum + (parseFloat(e.cost_usd as any) || 0), 0) || 0
    ...
    const recentActivity = (executions || [])
      ...
      .map(e => ({
        ...
        name: (e.workflow as any)?.name || 'Unknown Workflow',
