...
router.post('/', extractClerkUserId, async (req: Request, res: Response) => {
  ...
  const result = await resultsService.storeResult(
    workflow_id,
    project_id,
    clerk_user_id,
    title,
 ...
 */
router.post('/:resultId/share', async (req: Request, res: Response) => {
  const { resultId } = req.params
 ...
 */
router.post('/:resultId/notify', async (req: Request, res: Response) => {
  const { resultId } = req.params
