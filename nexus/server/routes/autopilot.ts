...
import { autopilotEngine, type WorkflowSpec } from '../services/AutopilotEngine.js'
...
router.post('/session', (req: Request, res: Response) => {
  try {
    const { workflowSpec, userId } = req.body as {
...
router.get('/stream/:sessionId', (req: Request, res: Response) => {
  ...
  const onScreenshot = (data: { sessionId: string; image: string; step: number; description: string; timestamp: number }) => {
  ...
  const onStatus = (data: { sessionId: string; state: string; step: number; message: string; timestamp: number }) => {
    if (data.sessionId !== sessionId) return
  ...
  const onProgress = (data: { sessionId: string; step: number; total: number; message: string; timestamp: number }) => {
    if (data.sessionId !== sessionId) return
  ...
  const onComplete = (data: { sessionId: string; steps: unknown[]; duration: number; timestamp: number }) => {
    if (data.sessionId !== sessionId) return
  ...
  const onError = (data: { sessionId: string; message: string; timestamp: number }) => {
    if (data.sessionId !== sessionId) return
...
router.get('/:sessionId/screenshot', (req: Request, res: Response) => {
  const { sessionId } = req.params
