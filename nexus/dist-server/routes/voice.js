 ...
 */
router.get('/profiles/filter', (req, res) => {
    try {
...
router.post('/config', async (req, res) => {
    try {
        const userId = req.headers['x-user-id'] || 'default';
        const { domain, language, preferredGender, voiceIdEn, voiceIdAr, customSettings, } = req.body;
        // Validate domain
...
router.post('/workflow/:workflowId', async (req, res) => {
    try {
        const { workflowId } = req.params;
        const { nodeId, voiceId, language, tone, customPrompt, settings, } = req.body;
 ...
 */
router.post('/call/config', async (req, res) => {
    try {
        const { preferences, workflowConfig, context, } = req.body;
        ...
        const callConfig = ElevenLabsVoiceService.buildVoiceCallConfig(preferences, workflowConfig, context);
        res.json({
            success: true,
            callConfig,
        });
 ...
 */
router.get('/call/url', async (_req, res) => {
    try {
