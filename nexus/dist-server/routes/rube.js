 ...
 */
function searchToolCatalog(useCase, toolkit) {
    const results = [];
    ...
    for (const [toolkitName, tools] of Object.entries(catalogToSearch)) {
        for (const tool of tools) {
            let score = 0;
            // Toolkit name match
            if (keywords.some((k) => toolkitName.includes(k)))
                score += 30;
            // Action match
            ...
            for (const keyword of keywords) {
                if (nameWords.includes(keyword))
                    score += 15;
                if (descWords.some((w) => w.includes(keyword)))
                    score += 10;
            }
            if (score > 0) {
                results.push({
    ...
    try {
        ...
        for (const query of queries) {
            ...
            if (query.known_fields) {
                ...
                if (toolkitMatch)
                    toolkit = toolkitMatch[1].toLowerCase();
            }
 ...
 */
function generateGenericSchema(toolSlug) {
