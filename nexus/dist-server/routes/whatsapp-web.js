...
import { whatsAppBaileysService as whatsAppWebService } from '../services/WhatsAppBaileysService';
const router = Router();
...
router.get('/qr/:id', async (req, res) => {
    ...
    const onQR = (sessionId, qrCode) => {
        if (sessionId === id) {
            sendEvent('qr', { qrCode });
        }
    };
    const onPairingCode = (sessionId, code) => {
        if (sessionId === id) {
            sendEvent('pairingCode', { code });
        }
    };
    const onAuthenticated = (sessionId) => {
        if (sessionId === id) {
            sendEvent('authenticated', { message: 'Successfully authenticated' });
        }
    };
    const onReady = (sessionId, phoneNumber) => {
        if (sessionId === id) {
    ...
    const onStateChanged = (sessionId, state) => {
        if (sessionId === id) {
            const currentSession = whatsAppWebService.getSession(id);
            sendEvent('state', {
                state,
    ...
    const onDisconnected = (sessionId, reason) => {
        if (sessionId === id) {
