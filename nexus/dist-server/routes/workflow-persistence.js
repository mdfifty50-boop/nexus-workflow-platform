 ...
 * POST /workflows/:id/executed - Increment execution count
 */
router.post('/workflows/:id/executed', async (req, res) => {
    const userId = getUserId(req);
    const { id } = req.params;
    if (!supabase) {
        return res.status(503).json({ error: 'Supabase not configured' });
...
router.patch('/executions/:id', async (req, res) => {
    ...
    try {
        ...
        if (updates.executionData !== undefined)
            dbUpdates.execution_data = updates.executionData;
        if (updates.tokenUsage !== undefined)
            dbUpdates.token_usage = updates.tokenUsage;
        if (updates.costUsd !== undefined)
            dbUpdates.cost_usd = updates.costUsd;
        const { error } = await supabase
            .from('user_workflow_executions')
            .update(dbUpdates)
            .eq('id', id);
