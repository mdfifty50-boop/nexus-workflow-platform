...
import { whatsAppComposioService, } from '../services/WhatsAppComposioService.js';
...
const voiceResponsePreferences = new Map();
/**
 * Check if voice responses are enabled for a user
 */
function isVoiceResponseEnabled(phone) {
 ...
 */
function getVoiceResponseLanguage(phone, detectedLang) {
    const prefs = voiceResponsePreferences.get(phone);
 ...
 */
function formatWorkflowForWhatsApp(workflow, lang = 'en') {
    if (lang === 'ar') {
 ...
 */
async function processMessageWithNexus(from, messageText, contactName) {
    try {
        // Get the Nexus agent
...
`;
        ...
        const systemBlocks = [{ type: 'text', text: nexusAgent.personality + whatsappContext + catalogueContext }];
 ...
 */
async function executeWorkflow(from, workflow, lang) {
 ...
 */
async function handleIncomingMessage(messageValue) {
        ...
        switch (message.type) {
            ...
            case 'audio':
                ...
                if (message.audio?.id) {
                    ...
                    const voiceResult = await VoiceNoteHandler.processVoiceNote(message.audio.id, message.audio.mime_type || 'audio/ogg', 'auto' // Auto-detect Arabic or English
                    );
        ...
        if (pendingWorkflow) {
            ...
            if (orderService.isOrderMessage(messageText)) {
                ...
                try {
                    ...
                    const extraction = await orderService.extractOrderFromMessage(messageText, from, contactName, catalogueItems);
                        ...
                        if (order.totalAmount && order.totalAmount > 0) {
                            try {
                                ...
                                if (payResult.success && payResult.payment) {
                                    const paymentMsg = paymentLinkService.formatPaymentMessage((await paymentLinkService.getPaymentStatus(payResult.payment.paymentId)).payment, lang);
...
router.post('/connect', async (req, res) => {
    try {
        ...
        if (result.error) {
            ...
            const isWabaError = (errorStr.includes('WABA') ||
 ...
 * Check if user has a pending workflow
 */
router.get('/pending/:phone', (req, res) => {
    try {
        const { phone } = req.params;
 ...
 */
router.delete('/pending/:phone', (req, res) => {
    try {
        const { phone } = req.params;
 ...
 */
router.get('/voice-prefs/:phone', (req, res) => {
    try {
        const { phone } = req.params;
        const prefs = voiceResponsePreferences.get(phone);
        res.json({
 ...
 */
router.post('/voice-prefs/:phone', (req, res) => {
    try {
        const { phone } = req.params;
 ...
 */
router.delete('/voice-prefs/:phone', (req, res) => {
    try {
        const { phone } = req.params;
...
router.get('/orders', (req, res) => {
    try {
        ...
        if (phone && typeof phone === 'string')
            filters.customerPhone = phone;
        if (fromDate && typeof fromDate === 'string')
            filters.fromDate = new Date(fromDate);
        if (toDate && typeof toDate === 'string')
            filters.toDate = new Date(toDate);
        if (limit)
            filters.limit = parseInt(String(limit), 10);
        if (offset)
            filters.offset = parseInt(String(offset), 10);
...
router.put('/orders/:id/status', async (req, res) => {
    try {
        ...
        orderSheetSyncService.updateOrderStatusInSheet(id, status).then(syncResult => {
            if (syncResult.success) {
 ...
 */
router.post('/orders/sheet-config', (req, res) => {
    try {
...
//# sourceMappingURL=whatsapp-composio.js.map