 ...
 * Delete a meeting
 */
router.delete('/:meetingId', extractClerkUserId, async (req, res) => {
    const { meetingId } = req.params;
    const clerkUserId = req.body.clerk_user_id;
    if (!clerkUserId) {
        return res.status(401).json({ success: false, error: 'Authentication required' });
 ...
 */
router.post('/:meetingId/transcribe', async (req, res) => {
    const { meetingId } = req.params;
    const meeting = await meetingService.getMeeting(meetingId);
    if (!meeting) {
        return res.status(404).json({ success: false, error: 'Meeting not found' });
 ...
 */
router.post('/:meetingId/translate', async (req, res) => {
    const { meetingId } = req.params;
    const meeting = await meetingService.getMeeting(meetingId);
    if (!meeting) {
        return res.status(404).json({ success: false, error: 'Meeting not found' });
    ...
    const result = await meetingService.translateToEnglish(meetingId, meeting.transcript, meeting.transcript_language);
    if (!result.success) {
        return res.status(400).json({ success: false, error: result.error });
    }
 ...
 */
router.post('/:meetingId/extract-sops', async (req, res) => {
    const { meetingId } = req.params;
    const meeting = await meetingService.getMeeting(meetingId);
    if (!meeting) {
        return res.status(404).json({ success: false, error: 'Meeting not found' });
 ...
 */
router.post('/:meetingId/generate-workflow', extractClerkUserId, async (req, res) => {
    const { meetingId } = req.params;
    ...
    const result = await meetingService.generateWorkflowFromSOP(meeting.project_id, clerk_user_id, sop);
    if (!result.success) {
        return res.status(400).json({ success: false, error: result.error });
    }
    res.json({
 ...
 */
router.get('/search', extractClerkUserId, async (req, res) => {
    ...
    const results = await meetingService.searchTranscripts(clerkUserId, q, project_id);
    res.json({
        success: true,
        data: results,
