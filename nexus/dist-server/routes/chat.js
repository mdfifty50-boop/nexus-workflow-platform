        ...
        if (lastUserMessage?.content) {
            try {
                ...
                if (appDetection.detectedApps.length > 0) {
                    console.log(`[Chat] Detected apps: ${appDetection.detectedApps.map((a) => a.name).join(', ')}`);
                    ...
                    for (const app of appDetection.detectedApps) {
                        ...
                        const discoveryResult = appDetection.toolDiscoveryResults.find(r => r.toolName.toLowerCase() === app.name.toLowerCase() ||
...
router.post('/stream', chatRateLimiter, async (req, res) => {
    ...
    try {
        ...
        // === Finding #10: Prompt Injection Defense - Layer 6: Per-user message rate limiting ===
        const streamUserId = req.headers['x-user-id'] ||
            req.headers['x-clerk-user-id'] ||
            req.auth?.userId ||
            req.ip || 'anonymous';
        ...
        if (lastUserMessage?.content) {
            try {
                ...
                const appDetection = await appDetectionService.detectAndAnalyze(lastUserMessage.content, getUserEntityId(req));
                if (appDetection.detectedApps.length > 0) {
                    console.log(`[Chat/Stream] Detected apps: ${appDetection.detectedApps.map((a) => a.name).join(', ')}`);
                    toolContext = appDetection.contextEnrichment;
                    for (const app of appDetection.detectedApps) {
                        const discoveryResult = appDetection.toolDiscoveryResults.find(r => r.toolName.toLowerCase() === app.name.toLowerCase() ||
                            r.toolName.toLowerCase().includes(app.name.toLowerCase()));
