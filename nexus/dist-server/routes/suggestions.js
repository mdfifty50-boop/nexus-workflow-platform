...
router.get('/', extractClerkUserId, async (req, res) => {
    try {
        ...
        const suggestions = await aiSuggestionsService.getSuggestions(userId, status, parseInt(limit, 10));
 ...
 */
router.get('/stats', extractClerkUserId, async (req, res) => {
    try {
        const userId = req.body.clerk_user_id;
 ...
 */
router.get('/intelligence', extractClerkUserId, async (req, res) => {
    try {
        const userId = req.body.clerk_user_id;
 ...
 */
router.get('/patterns', extractClerkUserId, async (req, res) => {
    try {
        const userId = req.body.clerk_user_id;
 ...
 */
router.post('/:id/act', extractClerkUserId, async (req, res) => {
    try {
        const userId = req.body.clerk_user_id;
        const { id } = req.params;
        ...
        // Record the feedback
        await aiSuggestionsService.recordFeedback(id, userId, action, {
            rating,
            text: feedback_text,
            modifications
        });
        ...
        await userIntelligenceService.learnFromAction(userId, suggestionType, learningAction, {
 ...
 */
router.post('/generate', extractClerkUserId, async (req, res) => {
    try {
        const userId = req.body.clerk_user_id;
 ...
 */
router.get('/opportunities', extractClerkUserId, async (req, res) => {
    try {
        const userId = req.body.clerk_user_id;
        const { minConfidence = 0.85, limit = 5 } = req.query;
        const opportunities = await userIntelligenceService.getHighConfidenceOpportunities(userId, parseFloat(minConfidence));
        // Apply limit after fetching
        const limitedOpportunities = opportunities.slice(0, parseInt(limit, 10));
        res.json({
            success: true,
