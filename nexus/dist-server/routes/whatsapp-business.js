...
router.post('/register-key', async (req, res) => {
    try {
        ...
        const result = await aiSensyService.registerApiKey(userId, apiKey, phoneNumber, displayName);
        if (result.success) {
            res.json({
                success: true,
                account: {
 ...
 */
router.get('/catalogue/stats', (_req, res) => {
    try {
 ...
 */
router.get('/catalogue/categories', (_req, res) => {
    try {
 ...
 */
router.put('/catalogue/:id', (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, price, currency, imageUrl, category, availability, isVisible, url, sku } = req.body;
...
//# sourceMappingURL=whatsapp-business.js.map