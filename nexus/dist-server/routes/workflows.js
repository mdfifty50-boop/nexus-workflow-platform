 ...
 */
function validateAndNormalizeToolSlug(task) {
 ...
 */
function validateRequiredParams(task) {
    const toolSlug = task.config?.toolSlug;
    if (!toolSlug)
        return { valid: true }; // No toolSlug = skip param validation
 ...
 */
function autoFillParams(tasks, missingParamTasks, userInput) {
    const filledParams = [];
    const inputText = userInput || '';
    for (const missing of missingParamTasks) {
        const task = tasks.find((t) => t.id === missing.taskId);
        if (!task || !task.config)
            continue;
        if (!task.config.params)
            task.config.params = {};
 ...
 */
function generateDemoResult(toolSlug) {
    const demoResults = {
 ...
 */
function generateUserPrompt(missingFields) {
 ...
 */
function detectBroadRequest(userInput) {
 ...
 */
function initClarificationState(category) {
    return {
 ...
 */
function getNextClarificationQuestion(state) {
 ...
 */
function buildPlanFromClarification(category, answers) {
    const tasks = [];
    let taskId = 1;
 ...
 */
router.post('/:id/clarify', extractClerkUserId, async (req, res) => {
    try {
        ...
        const state = config.clarificationState;
        ...
        const category = config.clarificationCategory;
 ...
 */
router.post('/:id/cancel', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
 ...
 */
router.post('/:id/retry', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
 ...
 */
router.get('/:id/checkpoints', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        // Verify access
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
 ...
 */
router.post('/:id/start', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        // Verify access
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
 ...
 */
router.get('/:id/plan', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
        if (!workflow) {
 ...
 */
router.post('/:id/approve', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
        if (!workflow) {
 ...
 */
router.post('/:id/execute', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
        if (!workflow) {
        ...
        // =========================================================================
        const providedParams = req.body.providedParams;
        if (providedParams) {
        ...
        // =========================================================================
        const requiredIntegrations = plan.requiredIntegrations || [];
        ...
        if (missingParamTasks.length > 0) {
            ...
            const userInput = workflow.user_input;
 ...
 */
router.post('/:id/execute-coordinated', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
        if (!workflow) {
 ...
 */
router.get('/:id/nodes', extractClerkUserId, async (req, res) => {
    try {
        const clerkUserId = req.body.clerk_user_id;
        const workflow = await workflowService.getWorkflowById(req.params.id, clerkUserId);
        if (!workflow) {
