...
class AppDetectionService {
    ...
    async detectAndAnalyze(message, userId = 'default') {
        ...
        const toolDiscoveryResults = await toolDiscoveryService.discoverMultipleTools(detectedApps.map((a) => a.name), {
        ...
        const hasLimitedSupport = toolDiscoveryResults.some((r) => r.supportLevel === 'partial' || r.supportLevel === 'browser_only' || r.supportLevel === 'none');
        return {
            detectedApps,
            toolDiscoveryResults,
            contextEnrichment,
     ...
     */
    buildContextEnrichment(apps, results) {
        if (results.length === 0)
            return '';
        const lines = ['## TOOL AVAILABILITY CONTEXT (AUTO-DETECTED)'];
...
//# sourceMappingURL=AppDetectionService.js.map