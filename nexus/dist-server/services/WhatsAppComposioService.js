...
class WhatsAppComposioServiceClass {
     ...
     */
    async initiateConnection(redirectUrl) {
     ...
     */
    async sendNexusTemplate(templateKey, to, variables, language = 'en') {
        ...
        for (const entry of payload.entry) {
            for (const change of entry.changes) {
                if (change.field === 'messages') {
                    ...
                    await this.handleTemplateStatusWebhook(change.value);
                }
            }
        }
    }
     ...
     */
    async handleMessageWebhook(value) {
     ...
     */
    async handleTemplateStatusWebhook(value) {
     ...
     */
    updateConversationWindow(phone) {
        const normalized = this.normalizePhoneNumber(phone);
        const now = new Date();
     ...
     */
    async smartSend(to, text, fallbackTemplate, templateVariables) {
...
//# sourceMappingURL=WhatsAppComposioService.js.map