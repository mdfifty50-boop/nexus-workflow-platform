...
export const patternDetectionService = {
     ...
     */
    async detectAllPatterns(userId) {
     ...
     */
    detectPeakUsageTimes(executions) {
        const hourCounts = {};
     ...
     */
    detectRepeatedManual(executions) {
        const patterns = [];
        const manualCounts = {};
        for (const exec of executions) {
     ...
     */
    detectWorkflowSequences(executions) {
        const patterns = [];
        const sequences = {};
        // Sort by timestamp
        const sorted = [...executions].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
     ...
     */
    detectIntegrationCombos(executions) {
        const patterns = [];
        const comboCounts = {};
        for (const exec of executions) {
     ...
     */
    detectFailurePatterns(executions) {
        const patterns = [];
        const failureCounts = {};
        for (const exec of executions) {
     ...
     */
    detectTimeOfWeekPatterns(executions) {
        const dayCounts = {};
     ...
     */
    detectVolumePatterns(executions) {
        ...
        for (const pattern of patterns) {
            switch (pattern.type) {
                case 'repeated_manual':
                    recommendations.push(`Schedule "${pattern.data.workflow_name}" to run automatically at your peak hour`);
                    break;
                case 'workflow_sequence':
                    recommendations.push(`Combine "${pattern.data.first_workflow}" and "${pattern.data.second_workflow}" into a single workflow`);
                    break;
                case 'failure_pattern':
                    recommendations.push(`Fix the recurring issues in "${pattern.data.workflow_name}" - it's failing ${pattern.data.failure_rate}% of the time`);
                    break;
                case 'integration_combo':
                    recommendations.push(`You use ${pattern.data.integrations.join(' + ')} together often - consider a template for this combo`);
                    break;
            }
        }
     ...
     */
    async getRecentExecutions(userId, days = 30) {
        if (!supabase) {
