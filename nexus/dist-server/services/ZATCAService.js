...
class ZATCAServiceClass {
    ...
    sandboxUrl = 'https://gw-fatoora.zatca.gov.sa/sandbox';
    ...
    async createInvoice(params) {
        try {
            ...
            // Calculate totals
            const subtotal = lineItems.reduce((sum, item) => sum + (item.quantity * item.unitPrice - (item.discount || 0)), 0);
     ...
     */
    async submitInvoice(invoiceId, userId) {
        try {
    ...
    async generateComplianceReport(startDate, endDate) {
        try {
            const invoices = Array.from(this.invoices.values()).filter(i => i.issueDate >= startDate && i.issueDate <= endDate);
...
//# sourceMappingURL=ZATCAService.js.map