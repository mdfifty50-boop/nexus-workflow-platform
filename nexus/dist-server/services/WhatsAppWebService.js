...
class WhatsAppWebService extends EventEmitter {
    ...
    scheduleReconnect(sessionId) {
        ...
        const delay = Math.min(CONFIG.RECONNECT_BASE_DELAY * Math.pow(2, session.reconnectAttempts), CONFIG.RECONNECT_MAX_DELAY);
     ...
     */
    async getConnectionState(sessionId) {
        const client = this.clients.get(sessionId);
        if (!client)
            return null;
        try {
    ...
    async shutdown() {
        ...
        const destroyPromises = Array.from(this.sessions.keys()).map(sessionId => this.destroySession(sessionId));
...
//# sourceMappingURL=WhatsAppWebService.js.map