...
class VoiceNoteHandlerClass {
     ...
     */
    async processVoiceNote(mediaId, mimeType, preferredLanguage = 'auto') {
        ...
        try {
            ...
            const transcription = await DeepgramService.transcribe(mediaInfo.url, preferredLanguage, {
                punctuate: true,
                smartFormat: true,
    ...
    async generateVoiceResponse(text, language = 'en') {
        try {
            ...
            const result = await ElevenLabsService.synthesizeForWhatsApp(text, language === 'ar' ? 'ar-AE' : 'en-US');
            if (!result.success) {
                return {
                    success: false,
     ...
     */
    async processVoiceConversation(mediaId, mimeType, processText, enableVoiceResponse = false) {
        // Step 1: Transcribe
    ...
    isSupported(mimeType) {
        return SUPPORTED_AUDIO_FORMATS.some(format => mimeType.toLowerCase().includes(format.split('/')[1]));
    }
    /**
...
//# sourceMappingURL=VoiceNoteHandler.js.map