...
Generate an execution plan as a JSON object.`;
            ...
            const claudeResult = await tieredCalls.planWorkflow(PLANNING_SYSTEM_PROMPT, userMessage, 4096);
        ...
        try {
            ...
            if (task.type === 'agent') {
                ...
                if (connectionStatus.connected) {
                    ...
                    const result = await composioService.executeToolForUser(userId, toolSlug, task.config);
                    if (result.success) {
                        output = {
                            success: true,
                            data: result.data,
                        ...
                        output = await this.executeViaRubeMCP(integrationId, toolSlug, task.config);
                    }
                }
                else {
                ...
                output = await this.executeDataTransform(task.config);
            }
     ...
     */
    async executeWithCoordination(workflowId) {
        try {
            // Get workflow using workflowService (supports in-memory storage)
            const workflow = await workflowService.getWorkflowById(workflowId, 'dev-user-local');
            if (!workflow) {
            ...
            const coordinationResult = await agentCoordinator.executeWorkflowWithCoordination(workflowId, plan.tasks.map((task) => ({
                id: task.id,
                name: task.name,
                type: task.type,
     ...
     */
    inferToolSlug(integrationId, taskName, _config) {
     ...
     */
    async executeViaRubeMCP(integrationId, toolSlug, params) {
        try {
     ...
     */
    async executeDataTransform(config) {
        try {
            ...
            switch (transformType) {
                ...
                case 'format':
                    ...
                    if (outputFormat === 'csv' && Array.isArray(inputData) && inputData.length > 0) {
                        ...
                        const rows = inputData.map(item => headers.map(h => String(item[h] || '')).join(','));
