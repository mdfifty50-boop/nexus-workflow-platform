...
export class WorkflowOrchestrator extends EventEmitter {
     ...
     */
    async executeWithRetry(fn, context, state) {
        let lastError = null;
    ...
    isNonRetryableError(error) {
        if (error instanceof Error) {
            return RETRY_CONFIG.nonRetryableErrors.some(code => error.message.toLowerCase().includes(code.toLowerCase()));
        }
        return false;
    }
    /**
     ...
     */
    async executeWorkflow(workflow, inputs = {}, options = {}) {
     ...
     */
    async executeStep(step, inputs, state) {
     ...
     */
    async executeRubeTool(toolSlug, params, state) {
     ...
     */
    async executePlaywrightAction(template, params, state) {
     ...
     */
    async executeEmbeddedAPI(endpoint, params, state) {
     ...
     */
    emitStepUpdate(state, stepId, status, result) {
     ...
     */
    emitWorkflowFailed(state, error) {
        this.emit('failed', {
            type: 'workflow_status',
            workflowId: state.workflowId,
            executionId: state.executionId,
     ...
     */
    getExecutionStatus(workflowIdOrExecutionId) {
