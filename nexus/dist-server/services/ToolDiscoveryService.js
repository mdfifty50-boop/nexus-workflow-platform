...
class ToolDiscoveryServiceClass {
     ...
     */
    async discoverTool(toolName, options = {}) {
        ...
        if (knownUnsupported) {
            ...
            const result = this.createUnsupportedResult(toolName, normalizedName, category, knownUnsupported, includeAlternatives);
        ...
        if (!composioResult.found) {
            ...
            let customIntegration = undefined;
        ...
        const availableActions = composioResult.tools.map((t) => t.toLowerCase());
        const expectedActions = COMMON_ACTIONS[category] || [];
        const missingActions = expectedActions.filter((action) => !availableActions.some((available) => available.includes(action)));
     ...
     */
    async discoverMultipleTools(toolNames, options = {}) {
     ...
     */
    async searchComposioForTool(toolName) {
        try {
     ...
     */
    createUnsupportedResult(toolName, normalizedName, category, knownInfo, includeAlternatives) {
     ...
     */
    getSuggestedAlternatives(category) {
        const suggestions = {
            CRM: [
