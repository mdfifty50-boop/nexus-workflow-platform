...
export const integrationService = {
     ...
     */
    generateAuthUrl(providerId, userId, projectId, redirectUri) {
     ...
     */
    async exchangeCodeForTokens(providerId, code, redirectUri, state) {
        const provider = INTEGRATION_PROVIDERS[providerId];
        if (!provider) {
     ...
     */
    async getAccessToken(userId, projectId, providerId) {
        const supabase = getSupabaseAdmin();
     ...
     */
    async checkHealth(credentialId) {
        const supabase = getSupabaseAdmin();
        const { data: credential, error } = await supabase
            .from('integration_credentials')
            .select('*')
     ...
     */
    async getProjectIntegrations(userId, projectId) {
        const supabase = getSupabaseAdmin();
        const { data, error } = await supabase
            .from('integration_credentials')
            .select('*')
     ...
     */
    async disconnectIntegration(userId, projectId, providerId) {
        const supabase = getSupabaseAdmin();
     ...
     */
    async getUsageAnalytics(userId, projectId) {
        ...
        for (const integration of integrations) {
            ...
            else if (integration.health_status === 'warning')
