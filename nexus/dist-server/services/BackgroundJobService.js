...
import { aiSuggestionsService } from './AISuggestionsService.js';
...
export const backgroundJobService = {
    ...
    scheduleJob(jobType) {
        ...
        this.runJob(jobType).catch(err => console.error(`[BackgroundJobs] Initial run of ${jobType} failed:`, err));
        const timer = setInterval(() => {
            this.runJob(jobType).catch(err => console.error(`[BackgroundJobs] Scheduled run of ${jobType} failed:`, err));
        }, interval);
     ...
     */
    async runPatternDetection(userId) {
     ...
     */
    async runDailySuggestions(userId) {
        ...
        for (const user of users) {
            try {
                ...
                const opportunities = await userIntelligenceService.getHighConfidenceOpportunities(user.id, MIN_SUGGESTION_CONFIDENCE);
     ...
     */
    async generateQualitySuggestions(context, intelligence, opportunities) {
...
- Most Used: ${intelligence.integrationAffinity.mostUsedIntegrations.slice(0, 3).map((i) => i.name).join(', ')}
...
Quality requirement: Only suggest if you're 90%+ confident this will help them.`;
     ...
     */
    async runWeeklyAnalysis(userId) {
     ...
     */
    async runAnalyticsRollup(userId) {
    ...
    async recordJobStart(jobId, jobType, userId) {
        if (!supabase)
            return;
        await supabase
    ...
    async recordJobComplete(jobId, result, durationMs) {
        if (!supabase)
            return;
        await supabase
            .from('background_jobs')
    ...
    async recordJobFailed(jobId, errorMessage, durationMs) {
        if (!supabase)
            return;
        await supabase
            .from('background_jobs')
     ...
     */
    async triggerForUser(userId, jobType) {
