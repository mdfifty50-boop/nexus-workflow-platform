...
function getCanonicalParamName(paramName) {
...
class PreFlightValidationServiceClass {
        ...
        if (this.composio) {
            try {
                ...
                if (tools && Array.isArray(tools)) {
                    ...
                    const tool = tools.find((t) => t.slug?.toUpperCase() === toolSlug.toUpperCase() ||
     ...
     */
    generateGenericSchema(toolSlug) {
     ...
     */
    isParamProvided(paramName, integration, nodeId, collectedParams, nodeParams, extractedParams) {
     ...
     */
    generateQuestion(paramName, paramInfo, integration, node, fallbackConfig) {
        ...
        const inputType = (fallbackConfig?.inputType || template?.inputType || 'text');
     ...
     */
    async check(nodes, collectedParams, connectedIntegrations) {
        const questions = [];
        const connections = [];
        const seenIntegrations = new Set();
        let schemaSource = this.composio ? 'composio' : 'fallback';
        ...
        for (const node of nodes) {
            ...
            if (schema.source === 'composio')
                usedComposio = true;
            if (schema.source === 'common' || schema.source === 'generic')
                usedFallback = true;
            // Check each required param
            for (const paramName of schema.required) {
                const isProvided = this.isParamProvided(paramName, integration, node.id, collectedParams, node.params, node.extractedParams);
                if (!isProvided) {
                    ...
                    const question = this.generateQuestion(paramName, schema.properties[paramName] || { type: 'string' }, integration, node, fallbackConfig);
        ...
        const seenCanonicalParams = new Set();
...
//# sourceMappingURL=PreFlightValidationService.js.map