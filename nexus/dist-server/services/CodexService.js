...
async function checkCodexHealth() {
    ...
    try {
        ...
        lastHealthCheck = now;
        console.warn('[CodexService] Health check error:', err.message);
        return false;
    }
}
// =============================================================================
...
// =============================================================================
async function callCodexAPI(useCase, input, context, overrides) {
...
// =============================================================================
async function fallbackToClaude(useCase, input, context) {
    const systemPrompt = USE_CASE_PROMPTS[useCase];
    const config = USE_CASE_CONFIG[useCase];
 ...
 */
export async function executeCodex(request) {
    const start = Date.now();
    const isHealthy = await checkCodexHealth();
    if (isHealthy) {
        try {
            const result = await callCodexAPI(request.useCase, request.input, request.context, { maxTokens: request.maxTokens, temperature: request.temperature });
            return {
                text: result.text,
            ...
            console.warn(`[CodexService] Codex failed for ${request.useCase}, falling back to Claude:`, err.message);
            // Fall through to Claude fallback
        }
    }
    // Fallback to Claude
    const result = await fallbackToClaude(request.useCase, request.input, request.context);
    return {
        text: result.text,
 ...
 */
export async function completeCode(input, context) {
