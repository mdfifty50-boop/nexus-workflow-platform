...
class AppointmentServiceClass {
    provider = 'calendly'; // Default to Calendly
     ...
     */
    async generateBookingLink(params, userId) {
        try {
            let result;
            const duration = params.duration || 30;
            if (this.provider === 'calendly') {
                result = await composioService.executeAction('CALENDLY_GET_USER_BUSY_TIMES', {
                ...
                result = await composioService.executeAction('CAL_CREATE_BOOKING_LINK', {
     ...
     */
    async createAppointment(params, userId) {
        try {
     ...
     */
    async sendConfirmation(appointment, userId) {
        try {
...
Thank you for choosing ${appointment.clinicName || 'our clinic'}!
     ...
     */
    async sendReminder(appointmentId, type, userId) {
        try {
...
Reply "CONFIRM" to confirm, or "CANCEL" to cancel.
     ...
     */
    async sendFeedbackRequest(appointmentId, userId) {
        try {
            const appointment = this.appointments.get(appointmentId);
            if (!appointment) {
                return { success: false, error: 'Appointment not found' };
...
Your feedback helps us improve!
     ...
     */
    async handlePatientReply(patientPhone, message, userId) {
        try {
            const normalizedMessage = message.toLowerCase().trim();
            // Find patient's upcoming appointment
            const appointment = Array.from(this.appointments.values()).find(a => a.patientPhone === patientPhone && ['scheduled', 'confirmed'].includes(a.status));
            if (!appointment) {
                return {
                    success: false,
                    action: 'unknown',
     ...
     */
    getAppointments(doctorEmail, status) {
     ...
     */
    getTodayAppointments(doctorEmail) {
    ...
    async syncToCalendar(appointment, userId) {
        try {
...
Managed by Nexus Appointment Service
    ...
    async scheduleReminders(appointment, userId) {
     ...
     */
    async runDailyReminderCheck(userId) {
        const now = new Date();
