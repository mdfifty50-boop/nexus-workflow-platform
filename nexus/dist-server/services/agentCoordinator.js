...
function getModelForAgent(agentId) {
    const modeName = (process.env.NEXUS_MODEL_MODE || modelModes.activeMode || 'default');
    const runtimeConfig = modelModes.runtimeAgents[modeName]
        || modelModes.runtimeAgents['default'];
    const tierName = runtimeConfig?.[agentId] || 'sonnet';
    const metadata = modelModes.modelMetadata;
...
export const agentCoordinator = {
     ...
     */
    async executeWithAgent(agentId, task, workflowId) {
...
Provide your response with the completed work.`;
     ...
     */
    async supervisorReview(taskResult, taskContext) {
...
- modifiedInput: (optional) if retrying with changes`;
            ...
            const claudeResult = await tieredCalls.classify(supervisor.systemPrompt, reviewPrompt, 1024);
            // Record metrics
            recordTieringMetrics(claudeResult.tier, claudeResult.taskType, claudeResult.costUSD, claudeResult.savingsVsSonnet);
     ...
     */
    async directorDelegate(task, workflowContext) {
     ...
     */
    async executeWorkflowWithCoordination(workflowId, tasks) {
        ...
        for (const task of sortedTasks) {
            ...
            while (attempts < maxAttempts) {
                attempts++;
                lastResult = await this.executeWithAgent(agentId, {
                    id: modifiedTask.id,
                ...
                if (decision.action === 'continue') {
                    ...
                    const escalatedResult = await this.executeWithAgent(decision.nextAgentId, {
                        id: modifiedTask.id,
                        name: modifiedTask.name,
                        description: modifiedTask.description,
                        input: modifiedTask.config,
