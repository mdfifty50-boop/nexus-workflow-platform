...
export const userIntelligenceService = {
     ...
     */
    async buildIntelligence(userId, forceRefresh = false) {
        ...
        const [profile, workflows, executions, suggestions, chatHistory, integrations] = await Promise.all([
        ...
        const automationMaturity = this.calculateAutomationMaturity(workflows.length, executionBehavior.successRate, workflowPatterns.avgStepsPerWorkflow);
        const primaryUseCase = this.inferPrimaryUseCase(workflows, integrationAffinity.mostUsedIntegrations, profile.industry);
        const painPoints = this.identifyPainPoints(executions, workflows, suggestionHistory, communicationStyle);
        const opportunities = this.identifyOpportunities(workflows, integrationAffinity, executionBehavior, suggestionHistory);
        // Calculate overall data quality
        const dataQuality = this.assessDataQuality(workflows.length, executions.length, chatHistory.length);
     ...
     */
    async getHighConfidenceOpportunities(userId, minConfidence = 0.85) {
     ...
     */
    async hasEnoughDataForSuggestions(userId) {
        const intelligence = await this.buildIntelligence(userId);
     ...
     */
    async learnFromAction(userId, suggestionType, action, context) {
    ...
    async fetchUserWorkflows(userId) {
        if (!supabase)
            return [];
        const { data } = await supabase
            .from('workflows')
    ...
    async fetchRecentExecutions(userId) {
        if (!supabase)
            return [];
    ...
    async fetchSuggestionHistory(userId) {
        if (!supabase)
            return [];
        const { data } = await supabase
            .from('ai_suggestions')
    ...
    async fetchIntegrations(userId) {
        if (!supabase)
            return [];
        const { data } = await supabase
    ...
    // =============================================================================
    analyzeWorkflowPatterns(workflows) {
        ...
        for (const wf of workflows) {
            ...
            // Count action types
            for (const step of steps.filter((s) => s.type === 'action')) {
    ...
    analyzeExecutionBehavior(executions) {
    ...
    analyzeIntegrationAffinity(integrations, executions, workflows) {
        const connected = integrations.map(i => i.toolkit || i.integration_name);
        // Count usage per integration
        const usageCounts = {};
        for (const exec of executions) {
            const tools = exec.workflows?.config?.steps?.map((s) => s.tool) || [];
        ...
        const combos = {};
        for (const wf of workflows) {
            const tools = (wf.config?.steps || []).map((s) => s.tool).sort();
            if (tools.length >= 2) {
    ...
    analyzeSuggestionHistory(suggestions) {
    ...
    analyzeCommunicationStyle(chatHistory) {
        ...
        const technicalCount = userMessages.reduce((count, m) => {
            return count + technicalTerms.filter(term => m.content?.toLowerCase().includes(term.toLowerCase())).length;
        }, 0);
    ...
    // =============================================================================
    calculateAutomationMaturity(workflowCount, successRate, avgSteps) {
        const score = (Math.min(workflowCount / 20, 1) * 30) +
            (successRate * 40) +
    ...
    inferPrimaryUseCase(workflows, mostUsedIntegrations, industry) {
    ...
    identifyPainPoints(executions, workflows, suggestionHistory, communicationStyle) {
        const painPoints = [];
    ...
    identifyOpportunities(workflows, integrationAffinity, executionBehavior, suggestionHistory) {
        const opportunities = [];
        ...
        for (const potential of integrationAffinity.potentialIntegrations.slice(0, 2)) {
            const relevance = this.calculateIntegrationRelevance(potential, integrationAffinity.mostUsedIntegrations.map(m => m.name));
            if (relevance > 0.7) {
                opportunities.push({
    ...
    inferPotentialIntegrations(usedIntegrations, connectedIntegrations) {
    ...
    calculateIntegrationRelevance(integration, usedIntegrations) {
    ...
    assessDataQuality(workflowCount, executionCount, chatCount) {
        const score = (Math.min(workflowCount / 5, 1) * 30) +
...
//# sourceMappingURL=UserIntelligenceService.js.map