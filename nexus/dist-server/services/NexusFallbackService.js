...
class NexusFallbackService {
     ...
     */
    getSuggestedAlternatives(missingToolkit) {
     ...
     */
    getFallbackConfig(originalToolkit, selectedFallback) {
     ...
     */
    getTransformations(originalToolkit, fallbackToolkit) {
        ...
        const transformationMap = {
            ...
            'slack→email': [
                {
                    field: 'channel',
                    transform: (channel) => `[Slack: ${channel}]` // Add context in subject
                },
                {
                    field: 'message',
                    transform: (message) => message // Keep message as-is
                }
            ],
            ...
            'trello→asana': [
                {
                    field: 'list',
                    transform: (list) => ({ section: list }) // Trello list → Asana section
                },
                {
                    field: 'labels',
                    transform: (labels) => labels.map(l => ({ tag: l })) // Trello labels → Asana tags
                }
            ]
        };
     ...
     */
    getConfidence(originalToolkit, fallbackToolkit) {
...
//# sourceMappingURL=NexusFallbackService.js.map