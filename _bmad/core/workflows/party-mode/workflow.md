...
- **Hired agents registry: `{project-root}/_agents/AGENTS.md`**

### Paths

- `installed_path` = `{project-root}/_bmad/core/workflows/party-mode`
- `agent_manifest_path` = `{project-root}/_bmad/_config/agent-manifest.csv`
- **`hired_agents_path` = `{project-root}/_agents/AGENTS.md`**
- **`hired_agents_folder` = `{project-root}/_agents/`**
- `standalone_mode` = `true` (party mode is an interactive workflow)

---

## HIRED AGENTS INTEGRATION

### Loading Hired Agents

**CRITICAL:** At party mode startup, ALWAYS load hired agents from the `_agents/` directory:

1. Read `{project-root}/_agents/AGENTS.md` to get list of hired agents
2. For each agent in the registry, load their full config from `_agents/[agent-slug].md`
3. Merge hired agents into the party roster alongside standard BMAD agents

### Hired Agent Format

Hired agents have researched expertise stored in markdown files with:
- **Metadata** (hire date, research sources, capture rate)
- **Identity** (who they are, what they know)
- **Core Expertise Areas** (with sources)
- **Decision Framework** (how they make decisions)
- **Guardrails** (what they NEVER do)
- **Quality Criteria** (good vs bad patterns)
- **Example Scenarios** (how they respond)

### Hired Agent Participation

When including a hired agent in discussion:
1. Read their full `.md` config file
2. Apply their expertise, guardrails, and decision frameworks
3. Respond in character with their documented communication style
4. Reference their specific knowledge sources when relevant

### Persistence Guarantee

Hired agents are **file-based** and persist:
- Across sessions
- Across PC restarts
- Across Claude context resets
- Forever (until manually deleted)

The `_agents/` folder is the **single source of truth** for hired expertise.
...
**Core BMAD Team:**
[Load agent roster from agent-manifest.csv and display all agents with icons]

**Hired Specialists (Research-Backed):**
[Load hired agents from _agents/AGENTS.md and display each with their expertise area and capture rate]

Example format for hired agents:
- 🎨 **OpenAI UI Engineer** - ChatGPT interface design, AI-first UX (~25-30% expertise captured)
